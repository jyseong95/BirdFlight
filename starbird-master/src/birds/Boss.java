...
public class Boss extends EnemyEntity implements Serializable{
	...
	private int shootFreq; // 총의 주파수?
	private int bulletID; // 총알
	private int offsetX; // 이벤트 대상 객체에서의 상대적 마우스 x좌표 위치를 반환
	private int offsetY; // 이벤트 대상 객체에서의 상대적 마우스 y좌표 위치를 반환
	private int bulletX; // 총알이 나가는 x좌표 위치?
	private int bulletY; // 총알이 나가는 x좌표 위치?
	private int speed; // 스피드
	
	private int offsetZig; 
	private int zigDest; // 목적지 변경?
	private boolean zigUp = false; // 위로 이동?
	private boolean zigDown = false; // 아래로 이동?
	private boolean engage = false; // 사로 잡다? 끌다?
	private boolean targetMode = false;
	private int strafeTimer = 1000; // 폭격 타이머
	private int targetTimer = 500; // 타겟 타이머
	...
	public void draw(Graphics g) {
		if(engage){
			...
			g.fillRect(xpos + xhploc, ypos + sizey, (int) maxHP, 5); // 꽉찬 도형 그릴때 (직사각형)
